/******************************************************************************/
/*            Generated by IBExpert 2022.3.8.1 25.05.2022 15:01:50            */
/******************************************************************************/
...
CREATE TABLE SKLAD (
    ...
    HOUSE_AGRESA      SMALLINT NOT NULL,
...
CREATE TABLE TOVAR (
    ...
    KOLICHESTVO                 SMALLINT NOT NULL,
    TSENA                       SMALLINT NOT NULL,
...
ALTER TABLE SKLAD ADD CONSTRAINT UNQ1_SKLAD UNIQUE (INDEKS_ADRESA);
ALTER TABLE TORGOVAYA_TOCHKA ADD CONSTRAINT UNQ1_TORGOVAYA_TOCHKA UNIQUE (INDEKS_TT);
ALTER TABLE TOVAR ADD CONSTRAINT UNQ1_TOVAR UNIQUE (NAME_TOVARA);
...
ALTER TABLE TOVAR_SKLADE ADD CONSTRAINT FK_TOVAR_SKLADE_1 FOREIGN KEY (ID_NUMBER_TOVARA) REFERENCES TOVAR (ID_NUMBER_TOVARA) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE TOVAR_SKLADE ADD CONSTRAINT FK_TOVAR_SKLADE_2 FOREIGN KEY (ID_NUMBER_SKLADA) REFERENCES SKLAD (ID_NUMBER_SKLADA) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE TOVAR_TT ADD CONSTRAINT FK_TOVAR_TT_1 FOREIGN KEY (ID_NUMBER_TOVARA) REFERENCES TOVAR (ID_NUMBER_TOVARA) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE TOVAR_TT ADD CONSTRAINT FK_TOVAR_TT_2 FOREIGN KEY (NAME_TT) REFERENCES TORGOVAYA_TOCHKA (NAME_TT) ON DELETE CASCADE ON UPDATE CASCADE;
...
begin
    for
    select ID_NUMBER_TOVARA, STOIMOST FROM TOVAR_SKLADE
    WHERE  ID_NUMBER_SKLADA=:pid_number_sklada
    INTO   :id_number_tovara,  :stoimost
  do
  BEGIN
  suspend;
end
...
begin
    for
    select ID_NUMBER_TOVARA, KOLICHESTVO FROM TOVAR_TT
    WHERE  NAME_TT=:pname_tt
    INTO   :id_number_tovara, :kolichestvo
  do
  BEGIN
  suspend;
end
...
begin
    for
    select ID_NUMBER_TOVARA, KOLICHESTVO, TSENA FROM TOVAR_TT
    WHERE  NAME_TT=:pname_tt
    INTO  :id_number_tovara, :kolichestvo, :TSENA
  do
  BEGIN
  if (TSENA>35) THEN TSENA =35 ;
  suspend;
end
END^


ALTER PROCEDURE SPISOK4 (
    PID_NUMBER_SKLADA INTEGER,
    PID_NUMBER_TOVARA INTEGER)
RETURNS (
    KOLICHESTVO SMALLINT,
...
begin
    for
    select KOLICHESTVO, TSENA, stoimost FROM TOVAR_SKLADE
    WHERE  ID_NUMBER_SKLADA=:pid_number_sklada and ID_NUMBER_TOVARA=:pid_number_tovara
    INTO  :kolichestvo, :TSENA  , :stoimost
  do
  BEGIN
  suspend;
end
...
N=5;
    for
    select FIRST (:N) ID_NUMBER_TOVARA, KOLICHESTVO, TSENA, stoimost FROM TOVAR_SKLADE
    WHERE  ID_NUMBER_SKLADA=:pid_number_sklada
    INTO  :id_number_tovara, :kolichestvo, :TSENA  , :stoimost
  do
  BEGIN
  suspend;
end
...
DESCRIBE FIELD HOUSE_AGRESA TABLE SKLAD
'>0';

DESCRIBE FIELD HOUSE_TT TABLE TORGOVAYA_TOCHKA
'>0';

DESCRIBE FIELD KOLICHESTVO TABLE TOVAR
'>0';

DESCRIBE FIELD TSENA TABLE TOVAR
'>0';
